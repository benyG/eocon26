      ...
      if (detector) {
        try {
          ...
          if (codes.length > 0) {
            ...
            if (raw && raw !== lastScannedRef.current && !cooldownRef.current) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050a0e", display: "flex", flexDirection: "column", alignItems: "center", padding: "16px", fontFamily: "'Share Tech Mono', monospace" }}>
      ...
      <div style={{ width: "100%", maxWidth: 480, display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        {scanCount > 0 && (
          <div style={{ background: "#00ff9d15", border: "1px solid #00ff9d30", borderRadius: 20, padding: "4px 12px" }}>
            <span style={{ color: "#00ff9d", fontSize: 12, fontWeight: "bold" }}>{scanCount} check-ins</span>
          </div>
        )}
      </div>
      ...
      <div style={{ width: "100%", maxWidth: 480, background: statusConfig.bg, border: `2px solid ${statusConfig.border}`, borderRadius: 20, overflow: "hidden", boxShadow: `0 0 40px ${statusConfig.border}40`, transition: "all 0.3s ease" }}>
                ...
                {[["0,0"], ["auto,0"], ["0,auto"], ["auto,auto"]].map(([pos], i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      [t === "0" ? "top" : "bottom"]: 0,
                      width: 32, height: 32,
                      borderTop: t === "0" ? "3px solid #00ff9d" : "none",
        ...
        {state !== "scanning" && (
          <div style={{ padding: 32, textAlign: "center", minHeight: 240, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
            ...
            {(state === "success" || state === "duplicate" || state === "error") && (
              <>
                <div style={{ fontSize: 52, color: statusConfig.color, marginBottom: 8 }}>{statusConfig.icon}</div>
                <div style={{ fontSize: 18, fontWeight: "bold", color: statusConfig.color, letterSpacing: 3, marginBottom: 16 }}>{statusConfig.title}</div>

                {state === "success" && result?.registration && (
                  <>
                    <div style={{ color: "#ffffff", fontSize: 22, fontWeight: "bold", fontFamily: "Georgia, serif", marginBottom: 8 }}>
                      {result.registration.fname} {result.registration.lname}
                    </div>
                    <span style={{ background: "#00ff9d20", color: "#00ff9d", padding: "4px 16px", borderRadius: 20, fontSize: 12 }}>
                      {result.registration.ticketType}
                    </span>
                    <div style={{ color: "#444", fontSize: 11, marginTop: 8 }}>
                      {new Date(result.registration.checkedInAt).toLocaleTimeString("fr-FR")}
                    </div>
                  </>
                )}

                {state === "duplicate" && result?.checkedInAt && (
                  <div style={{ color: "#888", fontSize: 12 }}>
                    Check-in déjà effectué le {new Date(result.checkedInAt).toLocaleString("fr-FR")}
                    {result.checkedInBy ? ` par ${result.checkedInBy}` : ""}
                  </div>
                )}

                {state === "error" && (
                  <div style={{ color: "#aaa", fontSize: 13 }}>{result?.error || "QR code non reconnu"}</div>
                )}
        ...
        {state === "scanning" && (
          <div style={{ padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid #1a2a1a" }}>
            ...
            <button onClick={stopCamera} style={{ background: "transparent", border: "1px solid #333", color: "#666", padding: "6px 16px", borderRadius: 8, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>
              Arrêter
            </button>
          </div>
        )}
...
function ManualInput({ onSubmit }: { onSubmit: (val: string) => void }) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); if (val.trim()) { onSubmit(val.trim()); setVal(""); } }}
      style={{ display: "flex", gap: 8 }}>
      <input
        value={val}
        onChange={e => setVal(e.target.value)}
        placeholder="Saisie manuelle du payload QR…"
      ...
      />
      <button type="submit" style={{ background: "#1a2a3a", border: "1px solid #2a3a4a", color: "#00ccff", padding: "8px 14px", borderRadius: 8, fontSize: 11, cursor: "pointer", fontFamily: "monospace" }}>
        OK
      </button>
    </form>
  );
